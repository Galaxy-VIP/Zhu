...
const WELCOME_CHANNEL_ID = '881207789379256400';
const LEAVE_CHANNEL_ID = '881207789379256400';
const GUILD_ID = '881207788297125909';
const ALL_MEMBERS_CHANNEL_ID = '927777894430810202';
const MEMBERS_CHANNEL_ID = '927777897798856724';
const BOT_CHANNEL_ID = '927777900860682281';
...
client.on("messageReactionAdd", async(react, user) => {
  ...
  try { 
    ...
    await member.roles.add(reactData.role)
      const channelChat = await client.channels.cache.get("887674414597013514")
      await channelChat.send({ content: `Selamat datang, ${member}! Sering-sering nimbrung disini ya biar aktif. Oh iya jangan lupa buat claim role di <#881207788922101824> ya! <:serval:1308339718475223060>`}).then(d => setTimeout(() => d.delete(), 60*1000))
...
const channelSent2 = "887674414597013514"
...
client.on('guildMemberAdd', async member => {
    await updateChannelNames(member.guild);
        ...
        await channel.send({
            content: `Welcome to this server, ${member}! go read the rules please <#892726906317844490>!`,
            files: [attachment]
        });
...
client.on('guildMemberUpdate', async (oldMember, newMember) => {
    try {
        ...
        if (oldBoostTimestamp !== newBoostTimestamp) {
            ...
            const thankYouChannelId = '1152142322511925329'; // Ganti dengan ID channel yang diinginkan
            const thankYouChannel = newMember.guild.channels.cache.get(thankYouChannelId);

            if (thankYouChannel) {
                let boostCount = client.guilds.cache.get('881207788297125909').premiumSubscriptionCount
                const embed = new MessageEmbed()
                    .setColor('#FFD700')
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  ...
  if (newState.channel && !oldState.channel) {
    ...
    const xpInterval = setInterval(async () => {
      let levelData = await levelSchema.findOne({ guild: "881207788297125909", id: userId });

      if (!levelData) {
        levelData = new levelSchema({
          guild: "881207788297125909",
          id: userId,
          xp: xpGive,
          level: 1
        });
        ...
        if (levelData.xp + xpGive >= xpNeeded) {
          ...
          const roleIdMap = {
            10: '1152818399366365215',
            20: '1152818715470082058',
            30: '1152819047101120534',
            50: '1152819292803448852',
            75: '1152819453642412172',
          };
          ...
          const channelLevel = "1264273155829268572";
...
client.on('messageCreate', async message => {
    ...
    let levelData = await levelSchema.findOne({ guild: "881207788297125909", id: message.author.id });

    if (!levelData) {
        levelData = await levelSchema.create({ guild: "881207788297125909", id: message.author.id, xp: 0, level: 0});
    }
    ...
    if (levelData.xp + xpGive >= xpRequire) {
        ...
        if(levelData.level == 10) {
            //ActiveRole
            if(message.member.roles.cache.has('1152818399366365215')) return;
            else await message.member.roles.add('1152818399366365215')
        }
        if(levelData.level == 20) {
            //VeryActiveRole
            if(message.member.roles.cache.has('1152818715470082058')) return;
            else await message.member.roles.add('1152818715470082058')
        }
        if(levelData.level == 30) {
            //HyperactiveRole
            if(message.member.roles.cache.has('1152819047101120534')) return;
            else await message.member.roles.add('1152819047101120534')
        }if(levelData.level == 50) {
            //SuperActiveRole
            if(message.member.roles.cache.has('1152819292803448852')) return;
            else await message.member.roles.add('1152819292803448852')
        }
        if(levelData.level == 75) {
            //SuperDuperActiveRole
            if(message.member.roles.cache.has('1152819453642412172')) return;
            else await message.member.roles.add('1152819453642412172')
        }
        const channelLevel = "1264273155829268572"
...
client.on("messageCreate", async message => {
    ...
    let channelImage = "892069484624478258"
    ...
    const members = oldState.channel?.members
        ...
        try {
            if(members.length > 0) {
              ...
              await voiceChannel.setName(`🎙️║ ${randomMember.user.globalName ? randomMember.user.globalName : randomMember.user.username }'s・voice`)
...
client.login("OTk5MTM0MTU1OTEwNjg4Nzcz.GrJLNC.ZzUO8bTV7jN6io8mB2LGM8lTr6oiW2axwmcR6A");
