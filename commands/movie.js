...
module.exports = {
  ...
  run: async (client, msg, args) => {
    ...
    if(!movieName) return msg.channel.send({ content: 'Please provide something to search' })
      ...
      .addField("Movie Title", `${result[0].title || "No title"}`)
      .addField("Movie ID", `${result[0].id || "???"}`)
      .addField("Release Date", `${result[0].release_date || "Unknown"}`)
      .addField("Language", `${result[0].original_language || "Unknown"}`)
      ...
      const m = await msg.reply({ content: `I found **${res.results.length}** movie matching`, embeds: [embed], components: [select]})
      ...
      collector.on("collect", async i => {
         if(i.customId === "movie-matching") {
            ...
            .addField("Movie Title", `${movieData.title || "No title"}`)
            .addField("Movie ID", `${movieData.id || "???"}`)
            .addField("Release Date", `${movieData.release_date || "Unknown"}`)
            .addField("Language", `${movieData.original_language || "Unknown"}`)
        ...
        collector.on("end", async i => {
          ...
          await m.edit({ components: [select]})
        })
      ...
      } else if(res.results.length === 0) return msg.reply({ content: `There's no result found **${res.results.length}**`, ephemeral: true})
    }).catch(e => {
      return msg.reply({ content: `Something went wrong \`\`\`${e.message}\`\`\``, ephemeral: true})})
  }
}
...
function generateStarRating(voteAverage) {
    ...
    const fullStars = '<:star22:1276033287416189081>'.repeat(starCount);
    const halfStar = ((voteAverage % 10) >= 5 && starCount < maxStars) ? '<:halfstar:1276032927519477790>' : '';
