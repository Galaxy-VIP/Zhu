...
module.exports = {
  ...
  options: [ 
  {
   ...
   execute: async(client, interaction, args) => {
     ...
     if(err) return interaction.reply({ content: `Got an error, make sure you get a message id from ${channel}`, ephemeral: true})
     ...
     if(reactionData) {
      ...
      return interaction.reply({ content: `This server already setup a reaction roles using ${emoji} on this message`, ephemeral: true})
    } else {
      let emojiErr;
      await messageId.react(emoji).catch(e => {
        emojiErr = e
     })
      
      if(emojiErr) return interaction.reply({ content: ":x: Could find with that emoji", ephemeral: true})
