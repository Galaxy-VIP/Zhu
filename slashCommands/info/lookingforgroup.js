  ...
  execute: async(client, interaction, args) => {
     ...
     await member.forEach(async act => {
      if(!act.presence || !act.presence.activities[0]) return
       var current = await act.presence.activities[0].name || await act.presence.activities[1].name || await act.presence.activities[2].name || await act.presence.activities[3].name
